...
function Refrigerator({select, setSelect, pos, id}) {
    ...
    const down = (e, sector) => {
        setSelect({...select,id:'Refrigerator', key:id, color, setColor, setRemoved});
    ...
    if(select.key === id) {
        document.onkeydown = function(e) {
            const newPosition = position.clone();
            const xlimit1 = [-19, -19, -17.5, -19];
            const xlimit2 = [18, 19.5, 19.5, 20];
            const zlimit1 = [-19, -19, -19, -17.5];
            const zlimit2 = [19.5, 18, 20, 19.5];
            let rot = rotation;
            if(e.key === 'ArrowUp') {
                newPosition.x -= 0.5;
                newPosition.z -= 0.5;
