...
import bodyUrl from '../objects/chair/body.obj'
import legUrl from '../objects/chair/leg.obj'

function Chair({select, setSelect, pos, id}) {
    const [position, setPosition] = useState(pos);
    const [rotation, setRotation] = useState(0);
    const [color, setColor] = useState('#333355');
    ...
    const LegMaterial = new THREE.MeshPhongMaterial({color: new THREE.Color('#99825e')});
    ...
    legmodel.children.forEach((mesh, i) => { 
        ...
        const edge = new THREE.LineSegments(
            new THREE.EdgesGeometry(mesh.geometry),
            new THREE.LineBasicMaterial({ color: (select&&select.key === id)?0xC71585:'#99825e', linewidth: 4 })
        ); 
        mesh.add(edge);
    });

    const down = (e, sector) => {
        setSelect({...select,id:'Chair', key:id, color, setColor, setRemoved});
    ...
    if(select.key === id) {
        document.onkeydown = function(e) {
            const newPosition = position.clone();
            const xlimit1 = [-24.5, -21, -17, -20.5];
            const xlimit2 = [17.5, 21, 25, 21];
            const zlimit1 = [-21, -24.5, -20.5, -17];
            const zlimit2 = [21, 17.5, 21, 25];
            let rot = rotation;
            if(e.key === 'ArrowUp') {
                newPosition.x -= 0.5;
                newPosition.z -= 0.5;
    ...
    return  (
        removed?<></>:
        <mesh castShadow scale={0.15} position={position} rotation-y={Math.PI/2-Math.PI*rotation/2}>
        <group>
