...
function Sofa({select, setSelect, pos, id}) {
    ...
    const [color, setColor] = useState('#335533');
    ...
    const down = (e, sector) => {
        setSelect({...select,id:'Sofa', key:id, color, setColor, setRemoved});
    ...
    if(select.key === id) {
        document.onkeydown = function(e) {
            const newPosition = position.clone();
            const xlimit1 = [-24.5, -14.5, -14.5, -14.5];
            const xlimit2 = [15, 15, 25, 15];
            const zlimit1 = [-14.5, -24.5, -14.5, -14.5];
            const zlimit2 = [15, 15, 15, 25];
            let rot = rotation;
            if(e.key === 'ArrowUp') {
                newPosition.x -= 0.5;
                newPosition.z -= 0.5;
    ...
    return  (
        removed?<></>:
        <mesh castShadow scale={0.13} position={position} rotation-y={Math.PI/2-Math.PI*rotation/2}>
        <group>
